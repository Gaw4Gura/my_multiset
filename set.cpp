...
template <class T>
inline void rotate(Node<T>*& o, int d) {
    Node<T>* k = o->ch[d ^ 1];
    o->ch[d ^ 1] = k->ch[d];
    k->ch[d] = o;
    o->maintain();
...
template <class T>
inline void insert(Node<T>*& o, T v) {
    if (!o) {
        o = new Node<T>(v);
        return;
    } else {
...
template <class T>
inline void erase(Node<T>*& o, T v) {
    if (!o) return;
    int d = o->cmp(v);
    if (d == -1) {
        if (o->w > 1)
...
inline int rank(Node<T>* o, T x) {
    ...
    if (d == -1)
        ...
        return rank(o->ch[1], x) + sz;
}

template <class T>
inline T decrease(Node<T>* o, T x) {
    int sz = rank(o, x);
    return kth(o, sz);
}

template <class T>
inline T increase(Node<T>* o, T x) {
    int sz = rank(o, x);
    sz += find(o, x) + 1;
    return kth(o, sz);
}
...
int main() {
    ...
    printf("After 3 is : ");
    printf("%d\n", increase(s, 3));
    printf("Detail usage please read the readme.md\n");
    return 0;
}
